...
namespace CoreValidatorExample.APILibrary.ValidationFactoryConcept.Factory
{
    public static class CustomObjectFactoryFacConcept
    {

        public static IValidatorFacConcept<T> GetObjectInstance<T>()
        {
            ...
            {
                case "EmployeeExample":
                    objInstance = (IValidatorFacConcept<T>?)Activator.CreateInstance(typeof(EmployeeValidatorWithFactory));
                    break;
                case "PersonExample":
                    objInstance = (IValidatorFacConcept<T>?)Activator.CreateInstance(typeof(PersonValidatorWithFactory));
                    break;
            }
            if (objInstance == null)
            {
